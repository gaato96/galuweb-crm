    ...
    return (
        <div className="p-6 space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-foreground">Finanzas y Control Mensual</h2>
                    <p className="text-sm text-muted-foreground">Flujo de caja, cobros parciales y gastos fijos</p>
                </div>

                <div className="flex items-center gap-3">
                    {/* Control de Mes */}
                    <div className="flex items-center gap-2 bg-card border border-border rounded-xl p-1 shadow-sm">
                        <button onClick={prevMes} className="p-2 hover:bg-secondary rounded-lg transition-colors text-muted-foreground hover:text-foreground">
                            <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button onClick={resetHoy} className="px-3 py-1 text-sm font-bold text-foreground hover:text-primary transition-colors">
                            {MESES[mesActual]} {anioActual}
                        </button>
                        <button onClick={nextMes} className="p-2 hover:bg-secondary rounded-lg transition-colors text-muted-foreground hover:text-foreground">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-bold hover:opacity-90 active:scale-95 transition-all shadow-lg shadow-primary/20"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5 relative overflow-hidden">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Ingresos Cobrados</span>
                        <div className="w-8 h-8 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400">
                            <DollarSign className="w-4 h-4" />
                        </div>
                    </div>
                    <p className="text-3xl font-black text-emerald-400">{formatCurrency(ingresosCobradosMes)}</p>
                    <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1">
                        <ArrowUpRight className="w-3 h-3 text-emerald-400" /> Efectivamente recibidos en {MESES[mesActual]}
                    </p>
                </div>

                <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5 relative overflow-hidden">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold text-rose-400 uppercase tracking-wider">Gastos Totales</span>
                        <div className="w-8 h-8 rounded-xl bg-rose-500/20 flex items-center justify-center text-rose-400">
                            <TrendingUp className="w-4 h-4" />
                        </div>
                    </div>
                    <p className="text-3xl font-black text-rose-400">{formatCurrency(totalGastosMes)}</p>
                    <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1">
                ...
                <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 relative overflow-hidden">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">Por Cobrar del Mes</span>
                        <div className="w-8 h-8 rounded-xl bg-amber-500/20 flex items-center justify-center text-amber-400">
                            <Clock className="w-4 h-4" />
                        </div>
                    </div>
                    <p className="text-3xl font-black text-amber-400">{formatCurrency(ingresosPendientesMes)}</p>
